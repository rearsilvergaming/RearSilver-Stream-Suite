...
bool SpotifyClient::searchTrack(const std::string &query, SpotifyQueueTrack &track){
	std::lock_guard<std::mutex> operationLock(m_operationMutex); track={}; if(query.empty())return false;
	std::string token;int64_t expiry;{std::lock_guard<std::mutex>lock(m_mutex);token=m_accessToken;expiry=m_expiresAt;}
	if((token.empty()||expiry<=nowSeconds()+60)&&!refreshAccessToken())return false;
	{std::lock_guard<std::mutex>lock(m_mutex);token=m_accessToken;}
	std::string trackId;
	if(query.rfind("spotify:track:",0)==0)trackId=query.substr(14);
	else if(const auto marker=query.find("open.spotify.com/track/");marker!=std::string::npos){trackId=query.substr(marker+23);const auto end=trackId.find_first_of("?& /\t\r\n");if(end!=std::string::npos)trackId.resize(end);}
	const std::wstring url=trackId.empty()
		?std::wstring(L"https://api.spotify.com/v1/search?q=")+wide(urlEncode(query))+L"&type=track&limit=1"
		:std::wstring(L"https://api.spotify.com/v1/tracks/")+wide(urlEncode(trackId));
	auto r=request(L"GET",url,token);if(r.status==401&&refreshAccessToken()){{std::lock_guard<std::mutex>lock(m_mutex);token=m_accessToken;}r=request(L"GET",url,token);}
	spotifyLog("Track search HTTP "+std::to_string(r.status));
	if(r.status!=200){std::lock_guard<std::mutex>lock(m_mutex);m_state.error="Spotify search failed (HTTP "+std::to_string(r.status)+").";return false;}
	JsonObject root;if(!JsonObject::TryParse(wide(r.body),root))return false;
	if(!trackId.empty())track=parseTrack(root);
	else {if(!root.HasKey(L"tracks"))return false;auto tracks=root.GetNamedObject(L"tracks");if(!tracks.HasKey(L"items"))return false;auto items=tracks.GetNamedArray(L"items");if(!items.Size())return false;track=parseTrack(items.GetAt(0));}
	return !track.uri.empty();
}

bool SpotifyClient::addToQueue(const std::string &uri){std::lock_guard<std::mutex>operationLock(m_operationMutex);if(uri.empty())return false;std::string token;int64_t expiry;{std::lock_guard<std::mutex>lock(m_mutex);token=m_accessToken;expiry=m_expiresAt;}if((token.empty()||expiry<=nowSeconds()+60)&&!refreshAccessToken())return false;{std::lock_guard<std::mutex>lock(m_mutex);token=m_accessToken;}const std::wstring url=std::wstring(L"https://api.spotify.com/v1/me/player/queue?uri=")+wide(urlEncode(uri));auto r=request(L"POST",url,token);if(r.status==401&&refreshAccessToken()){{std::lock_guard<std::mutex>lock(m_mutex);token=m_accessToken;}r=request(L"POST",url,token);}spotifyLog("Add to queue HTTP "+std::to_string(r.status));if(r.status>=200&&r.status<300)return true;std::lock_guard<std::mutex>lock(m_mutex);m_state.error="Spotify could not add the track to its queue (HTTP "+std::to_string(r.status)+").";return false;}
