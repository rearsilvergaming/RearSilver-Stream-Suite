...
TwitchChatService::TwitchChatService(std::string label):m_label(std::move(label)){}
TwitchChatService::~TwitchChatService(){disconnect();}
void TwitchChatService::note(const std::string&m){SYSTEMTIME t{};GetLocalTime(&t);char stamp[16]{};sprintf_s(stamp,"%02u:%02u:%02u",t.wHour,t.wMinute,t.wSecond);std::lock_guard<std::mutex>l(m_mutex);m_diagnostics+=std::string(stamp)+"  "+m_label+": "+m+"\r\n";if(m_diagnostics.size()>10000)m_diagnostics.erase(0,m_diagnostics.size()-10000);}
...
void TwitchChatService::disconnect(){m_stop=true;interrupt("IRC disconnect requested");if(m_thread.joinable())m_thread.join();setConnected(false,"IRC disconnected");std::lock_guard<std::mutex>l(m_mutex);m_socket=nullptr;}
void TwitchChatService::setConnected(bool connected,const std::string&reason){if(m_connected.exchange(connected)==connected)return;++m_revision;note(reason);}
void TwitchChatService::interrupt(const std::string&reason){if(m_interrupting.exchange(true))return;setConnected(false,reason);void*s=nullptr;{std::lock_guard<std::mutex>l(m_mutex);s=m_socket;}if(s)WinHttpWebSocketClose((HINTERNET)s,WINHTTP_WEB_SOCKET_SUCCESS_CLOSE_STATUS,nullptr,0);}
void TwitchChatService::tick(){const uint64_t last=m_lastActivity.load();if(!last)return;bool hasSocket=false;{std::lock_guard<std::mutex>l(m_mutex);hasSocket=m_socket!=nullptr;}if(!hasSocket)return;const uint64_t timeout=m_connected?360000:45000;if(GetTickCount64()-last>timeout)interrupt(m_connected?"IRC connection became stale; reconnecting":"IRC join confirmation timed out; reconnecting");}
bool TwitchChatService::sendRaw(const std::string&line){DWORD error=ERROR_INVALID_HANDLE;{std::lock_guard<std::mutex>l(m_mutex);if(m_socket)error=WinHttpWebSocketSend((HINTERNET)m_socket,WINHTTP_WEB_SOCKET_UTF8_MESSAGE_BUFFER_TYPE,(void*)line.data(),DWORD(line.size()));}if(error==NO_ERROR)return true;interrupt("IRC send failed ("+std::to_string(error)+"); reconnecting");return false;}
...
void TwitchChatService::run(std::string login,std::string token,std::string channel,std::string broadcasterId,Handler handler){
	for(int attempt=0;!m_stop;++attempt){
		m_interrupting=false;m_lastActivity=GetTickCount64();
		HINTERNET session=WinHttpOpen(L"RearSilverStreamSuite/1.0",WINHTTP_ACCESS_TYPE_AUTOMATIC_PROXY,nullptr,nullptr,0);
		if(session)WinHttpSetTimeouts(session,10000,10000,15000,360000);
		...
		{std::lock_guard<std::mutex>l(m_mutex);m_socket=socket;}m_lastActivity=GetTickCount64();
		if(!sendRaw("PASS oauth:"+token+"\r\n")||!sendRaw("NICK "+login+"\r\n")||!sendRaw("CAP REQ :twitch.tv/tags twitch.tv/commands\r\n")||!sendRaw("JOIN #"+channel+"\r\n")){note("IRC login commands failed");}
		note("IRC socket opened as "+login+" for #"+channel+"; awaiting join confirmation");
		std::string pending;char buffer[8192];
		bool reconnectRequested=false;
		while(!m_stop&&!reconnectRequested){DWORD read=0;WINHTTP_WEB_SOCKET_BUFFER_TYPE type;DWORD e=WinHttpWebSocketReceive(socket,buffer,sizeof(buffer),&read,&type);if(e!=NO_ERROR||type==WINHTTP_WEB_SOCKET_CLOSE_BUFFER_TYPE)break;m_lastActivity=GetTickCount64();pending.append(buffer,read);if(type==WINHTTP_WEB_SOCKET_UTF8_FRAGMENT_BUFFER_TYPE||type==WINHTTP_WEB_SOCKET_BINARY_FRAGMENT_BUFFER_TYPE)continue;size_t end=0;while((end=pending.find("\r\n"))!=std::string::npos){std::string line=pending.substr(0,end);pending.erase(0,end+2);if(line.rfind("PING",0)==0){sendRaw("PONG"+line.substr(4)+"\r\n");continue;}if(line.find(" RECONNECT")!=std::string::npos){note("IRC server requested reconnect");reconnectRequested=true;break;}if(line.find(" NOTICE * :Login authentication failed")!=std::string::npos||line.find(" NOTICE * :Improperly formatted auth")!=std::string::npos){note("IRC authentication rejected");reconnectRequested=true;break;}if(line.find(" ROOMSTATE #"+channel)!=std::string::npos||line.find(" 366 "+login+" #"+channel)!=std::string::npos){setConnected(true,"IRC authenticated and joined #"+channel);attempt=0;}size_t priv=line.find(" PRIVMSG #");if(priv==std::string::npos||line.empty()||line[0]!='@'||!handler)continue;size_t tagEnd=line.find(' '),textPos=line.find(" :",priv);if(tagEnd==std::string::npos||textPos==std::string::npos)continue;auto t=tags(line.substr(1,tagEnd-1));TwitchChatMessage m;m.userId=t["user-id"];m.displayName=t["display-name"];m.text=line.substr(textPos+2);m.subscriber=t["subscriber"]=="1"||badge(t["badges"],"subscriber")||badge(t["badges"],"founder");m.vip=badge(t["badges"],"vip");m.moderator=t["mod"]=="1"||badge(t["badges"],"moderator");m.broadcaster=(!broadcasterId.empty()&&m.userId==broadcasterId)||badge(t["badges"],"broadcaster");handler(m);}}
		setConnected(false,m_stop?"IRC stopped":"IRC transport lost; retrying");{std::lock_guard<std::mutex>l(m_mutex);m_socket=nullptr;}if(!m_interrupting.exchange(true))WinHttpWebSocketClose(socket,WINHTTP_WEB_SOCKET_SUCCESS_CLOSE_STATUS,nullptr,0);WinHttpCloseHandle(socket);if(connection)WinHttpCloseHandle(connection);if(session)WinHttpCloseHandle(session);m_lastActivity=0;if(!m_stop){for(int i=0;i<std::min(100,10+attempt*10)&&!m_stop;++i)Sleep(100);}
	}
}
