	  ...
	  m_botAuth(botAuth)
{
	...
	layout->addWidget(makeProviderStatus("YouTube / YouTube Music", "Managed by the Suite Media Player", true));
	layout->addWidget(makeProviderStatus("Spotify", "External-player connection not configured", false));
	layout->addWidget(makeProviderStatus("Local files", "Media Player library migration is next", true));
